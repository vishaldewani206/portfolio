...
const Navbar = ({ isDark, setIsDark, setIsLoad, isLoad }) => {
	...
	return (
		<div className="px-5 py-3  z-10">
			<div className="flex justify-between  items-center">
				<Link
					to={"/"}
					className="p-3 cursor-pointer border rounded-full"
				>
					<h1>
  ...
  flex md:flex-row flex-col justify-center  space-y-3 md:space-y-0 md:space-x-3
  ${open ? "flex" : "hidden"} md:flex`}
				>
					<Link
						...
						} hover:text-primary text-pretty`}
						to={"/"}
					>
						Home
					</Link>
